...
async def _test_openai_compat(client: httpx.AsyncClient, cfg: LLMConfig) -> dict:
    try:
        ...
        return {"success": False, "message": f"连接 {cfg.get_base_url()} 超时"}
    except httpx.ConnectError:
        return {"success": False, "message": f"无法连接到 {cfg.get_base_url()}"}
    ...
    if resp.status_code == 200:
        ...
        return {"success": False, "message": f"API Key 无效 ({resp.status_code})"}
    elif resp.status_code == 429:
        return {"success": False, "message": "请求过于频繁，请稍后重试"}
    else:
        body = resp.text[:500]
        return {"success": False, "message": f"服务器返回 {resp.status_code}: {body}"}
